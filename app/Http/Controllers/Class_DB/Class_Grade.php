...
class Class_Grade
{
    ...
    {
        // set value variable
        $idGrade=''; $level=''; $nominalTnjTransport=''; $intervalBln=''; $nominalTnjJabatan=''; $intervalBlnJabatan='';
        ...
        if (isset($request['nominal_tnj_transport']) && $request['nominal_tnj_transport']!='' ) {$departemen = $request['nominal_tnj_transport'];}
        if (isset($request['interval_bln']) && $request['interval_bln']!='' ) {$intervalBln = $request['interval_bln'];}
        if (isset($request['nominal_tnj_jabatan']) && $request['nominal_tnj_jabatan']!='' ) {$nominalTnjJabatan = $request['nominal_tnj_jabatan'];}
        if (isset($request['interval_bln_jabatan']) && $request['interval_bln_jabatan']!='' ) {$intervalBlnJabatan = $request['interval_bln_jabatan'];}

        try
        {
            $data_ = DB::table('grade');
            if($idGrade!='')
            {
                $data_->where('id_grade',$idGrade);
            }
            if($level!='')
            {
